# Interview Prep Hub — Codex Project Instructions

This file gives Codex full context about this project so every conversation
...
1. Drop the file into the topic folder, e.g. `docs/java/hashmap-internals.html`
...
- `"hashmap-internals.html"` → resolves to `docs/java/hashmap-internals.html`
- `"reference/cheatsheet.html"` → resolves to `docs/java/reference/cheatsheet.html`
...
1. Add entry to `data.js` `topics` array (copy template, change id/title/accent/path/groups)
2. Create folder + `index.html` shell (copy any existing topic shell, change `window.TOPIC_ID`)
3. Add nav link to all 9 HTML nav bars (index.html + 8 topic pages)
4. Add accent class to `styles.css` if new colour is needed
...
2. Update `data.js` to point to it and mark as `available`
...
Java and Spring Boot concepts listed in `data.js` now have corresponding study pages.
...
- Same `:root` variables as `styles.css` (copy them inline or link `../styles.css`)
